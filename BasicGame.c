...
void space(void)		//(unsigned int size) // create space
{
    printf("     ");
}
...
void secondLine(unsigned int size, unsigned int playerPosition, char playerName)	//create the inside and return the type for the first line
{
    unsigned int x, j;
    for (j = 0; j < size; j++) {
	x = j;
...
void secondLine2nd(unsigned int size, unsigned int i,
		   unsigned int playerPosition, char playerName)
...
char getValidCharacter(void)	//make sure the character input is right//
{
    char choice;
    do {
	scanf("%c", &choice);
	getchar();
	if ((choice != 'p' && choice != 'q' && choice != 'r'
	     && choice != 's')) {
	    printf("Invalid input, please try again:  ");
	} else
	    break;
    } while (1);		//(choice != 'p' && choice != 'q' && choice != 'r' && choice != 's');
    return choice;
}
...
int getValidInteger(int low, int high)	//validate the input number//
{
    ...
    } while (1);		//(choice < low || choice > high);
    return choice;
}
...
void loseItem(int playerPrizes[], unsigned int *prizeCount)	// do the loseitem fuction
{
    int i, j, k, /*r, */ ran = 2;
...
void initPlayer(int *playerScore, int playerPrizes[], unsigned int *prizeCount, char *playerName, unsigned int *playerPosition)	//do the initplayer function, set everything to 0
{
    //int i;
    playerPrizes[MAX] = 0;
...
void playGame(unsigned int size, int *playerScore, int playerPrizes[], unsigned int *prizeCount, char *playerName, unsigned int *playerPosition)	//play the game
{
	...
	printf("Score: %d   inventory (%d items): ", *playerScore,
	       *prizeCount);
	for (i = 0; i < *prizeCount; i++) {
	...
	*playerPosition = *playerPosition + playerRoll(1, 6);
	...
	} else if (getDisplayType(*playerPosition, *playerPosition, '#') ==
		   'W') {
...
int main(void)
{
	...
	if (choice == 'p') {
	    ...
	    initPlayer(&playerScore, playerPrizes, &prizeCount,
		       &playerName, &playerPosition);

	    printf("Enter board size: ");
	    scanf("%d", &size);

	    playGame(size, &playerScore, playerPrizes, &prizeCount,
		     &playerName, &playerPosition);

...
char getDisplayType(unsigned int index, unsigned int playerPosition,
                    char playerName) // check the index and return character
{
...
void secondLine(unsigned int size, unsigned int playerPosition,
                char playerName) // create the inside and return the type for the first line
{
    unsigned int x, j;
    for(j = 0; j < size; j++)
    {
...
void middlerow(unsigned int size, unsigned int i, unsigned int playerPosition,
               char playerName) // create the inside and return the type for the square of the row
{
    unsigned int q, b, p;
    b = i;
    q = 3 * (size - 1) + ((size - 1) - i);
...
void initPlayer(int *playerScore, int playerPrizes[], unsigned int *prizeCount, char *playerName,
                unsigned int *playerPosition) // do the initplayer function, set everything to 0
{
    // int i;
    playerPrizes[MAX] = 0;
...
void playGame(unsigned int size, int *playerScore, int playerPrizes[], unsigned int *prizeCount, char *playerName,
              unsigned int *playerPosition) // play the game
{
